...
def remove_noise_peak_valley(times, sig, fs, peaks, amps,
    ...
    if len(sig) > window:
        ...
        for i in range(0, len(sig), window):
            ...
            # Remove noise when peaks distance is not stable (at least N peaks needed for N-sec window)
            if len(peaks_rs) <= n_peak_min:
                indicators[IMIN:IMAX] = 0
                continue
            # Remove noise when peaks distance is not stable (time intervalle between peaks < N seconds)
            is_rr_stable = True
