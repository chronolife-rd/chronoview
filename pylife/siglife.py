...
from pylife.useful import get_peaks_clean_unwrap
...
from pylife.remove import remove_noise_peak_valley_unwrap
...
class Breath(SiglifePeriodic):
    ...
    def clean(self):
        ...
        mpd = int(fs*1)
        peaks, amps = detect_peaks_unwrap(sig, mph=0, mpd=mpd, threshold=0, edge='rising',
                     kpsh=False, valley=False)
        
        peaks, amps = detect_valid_peaks_breath_unwrap(sig, fs, peaks)
        
        # Step 1: Clean for breath rate processing
        ...
        # Step 1 bis: Clean for Apnea detection
        _, _,\
        ...
        self.peaks_                     = peaks
        self.peaks_times_               = get_peaks_times_unwrap(times, peaks)
        self.peaks_amps_                = amps
        self.peaks_amps_mv_             = convert_mv_unwrap(self.peaks_amps_, self.breath_gain_)
...
class ECG(SiglifePeriodic):
    ...
    def clean(self):
        ...
        self.peaks_qrs_times_           = get_peaks_times_unwrap(times, peaks_qrs)
        self.peaks_                     = peaks
        self.peaks_times_               = get_peaks_times_unwrap(times, peaks)
        self.peaks_amps_                = amps
    ...
    def analyze(self, on_sig=None):
        ...
        if len(unwrap(qt_features['q_start_index_s'])) > 0:
            ...
            self.qt_length_median_clean_    = [qt_features['qt_length_median_clean_']]
