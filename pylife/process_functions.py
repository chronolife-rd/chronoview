...
def process_data_interval(dict_params):
   ...
   input dict_params = {
    ...
    if not dl.accs.is_empty_:
        ...
        dict_result["activity_level_times"]     = [time_sequence[0].astype(datetime.datetime).replace(tzinfo=datetime.timezone.utc)
    ...
    if not dl.breath_1.is_empty_:
        ...
        dict_result["breath_1_filtered_times"]          = [time_sequence[0].astype(datetime.datetime).replace(tzinfo=datetime.timezone.utc)
                                                            for time_sequence in dl.breath_1.times_]
    ...
    if not dl.breath_2.is_empty_:
        ...
        dict_result["breath_2_filtered_times"]          = [time_sequence[0].astype(datetime.datetime).replace(tzinfo=datetime.timezone.utc)
    ...
    if not dl.ecg.is_empty_:
        ...
        dict_result["ecg_filtered_times"]           = [time_sequence[0].astype(datetime.datetime).replace(tzinfo=datetime.timezone.utc)
                                                   for time_sequence in dl.ecg.times_]
    ...
    if not dl.temp_1.is_empty_:
        dict_result["averaged_temp_1"] = round(dl.temp_1.mean_*1e2)/1e2
        
    if not dl.temp_2.is_empty_:
        dict_result["averaged_temp_2"] = round(dl.temp_2.mean_*1e2)/1e2
