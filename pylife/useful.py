...
def compute_rr_features_unwrap(peaks_times, peaks, fs):
    ...
    if len(rr_interval) > 0:
        ...
        rmssd_s.append(np.NaN)
        lnrmssd_s.append(np.NaN)
...
def detect_qt_interval(siglist, fs):
...
def compute_qt_times(ecg_sig_clean_list : list,
                               ecg_time_clean_list : list, fs : int):
    ...
    #detect qt indexes
    q_start_index_s = detect_qt_interval(ecg_sig_clean_list, fs)[0]
    t_stop_index_s = detect_qt_interval(ecg_sig_clean_list, fs)[1]
    ...
    #unwrap qt length to calc mean and std
    qt_length_unwrap = []
    qt_length_ms    = []
    for qt in qt_length_s :
        qt_length_unwrap        = qt_length_unwrap + qt   
        qt_length_ms.append(np.array(qt).astype(float)/1e3)
        
    qt_length_unwrap            = np.array(qt_length_unwrap)
    qt_length_unwrap_int        = [qti.astype('int') for qti in qt_length_unwrap]
    qt_length_unwrap_int        = [round(qti*(1/1000),0) for qti in qt_length_unwrap_int]
    ...
    if (len([qti for qti in qt_length_unwrap_int if 350<qti<550]))>0:
        qt_length_med_clean         = np.median([qti for qti in qt_length_unwrap_int if 350<qti<550])
    else :
        qt_length_med_clean         = np.NaN
    ...
    output['qt_length_median_clean_']   = qt_length_med_clean
